function [flowThick, deposThick] = fanLobeDeposition(glob, topog) 
    ...
    deposDone = false; 
    % update current cell coordinate acconding to the new, increased
    % matrices size
    ...
    while deposDone == false %&& glob.emt <= 500
       [newFlowThick, newDeposThick] = fanLobeDepositionStep(flowThick, deposThick, glob, dummyTopog);
       
       newYSize = size(flowThick,1);
       newXSize = size(flowThick,2);
       if isempty(find(newFlowThick(2:newYSize-1,2:newXSize-1) > glob.minFlowThick, 1))
          deposDone = true;
       end
       ...
       glob.fracDepos(glob.sedType) = glob.fracDepos(glob.sedType) + glob.depositSlopeFactor(glob.sedType); 

       glob.emt = glob.emt + 1;
    end

    %Resize matrices according to the model size  
...
%
% Input:          - fracDepos: fraction of the total sediment to be
%                             deposited in every cell.
%
%                 - grads: grads(i,j,1) contains the gradient between
%                          cell(i,j) and east cell(i,j+1) and so on
%                          (clockwise).
%
%                 - gradNbr: vector containing cell(i,j) neighbours
%                            gradient.
%
...
flowThickCost(deposThick > 0) = 0;  % the flow cannot go back into previous occupied cells                                  
   %% Main function loop:
    ...
    % is then stored in the flowThick matrix.

    for k = 1:size(row,1)   % NB: lavora con una cella alla volta!!!
            y = row(k);
            x = col(k);
