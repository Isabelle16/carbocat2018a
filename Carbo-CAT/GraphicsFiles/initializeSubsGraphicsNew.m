function initializeSubsGraphicsNew(glob,subs)
...
% plotLastSurface=sum(subs.subsidence(:,:,:),3); %% to create the initial topography, should be positive
...
ax = gca;
xlabel('X Distance (km)', 'FontSize',40,'FontWeight','bold');
ylabel('Y Distance (km)', 'FontSize',40,'FontWeight','bold');
zlabel('Z (m)','FontSize',40,'FontWeight','bold')
lx = smallxSize*(glob.dx*10^-3); 
ly = smallySize*(glob.dx*10^-3); 
ax.XTick = [1 subs.xSize/2 subs.xSize];
ax.XTickLabel = [0 lx/2 lx];
...
%% Plot cross sections position using patch
