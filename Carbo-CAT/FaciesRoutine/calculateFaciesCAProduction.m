function [glob] = calculateFaciesCAProduction(glob, iteration)
...
% The order in which facies are checked depends on the production value at
...
for y = 1 : glob.ySize
    for x= 1 : glob.xSize
        ...
        % water depth > 0.001 and if the concentration is enough to
        % maintain production
            ...
            elseif (oneFacies <= glob.maxProdFacies && oneFacies>0) && glob.numberOfLayers(y,x,j)==1
                ...
                for f = 1:glob.maxProdFacies
                    if tempFaciesProd(y,x,f) > 0
                        checkProcess=strcmp(glob.waveRoutine,'on');
                        if checkProcess ==1 
                            %If the wave energy routine is on, the facies colonise if the wave energy level is appropriate for each facies 
                            if glob.wd(stats.averagePos(iteration-1,x),x,iteration-1)> (3/0.78) %
                                ...
                                prodRate(f) = glob.prodRate(f) * calculateProductionWDAdjustment(glob, x,y, f, iteration) * calculateProductionWaveEnergyAdjustment(glob, x,y, f, iteration);
                            end
                        else %Wave energy routine is off
                            prodRate(f) = glob.prodRate(f) * calculateProductionWDAdjustment(glob, x,y, f, iteration);
                        end
                    end
                end
                
                %Find the highest production rate and assign a fraction of
                %1 at the correct position in the glob.fraction array
